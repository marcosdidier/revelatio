...
**Caminho principal — motor Python de referência (construído e validado).** O componente central é o pipeline em `06_reference_script/`, que implementa as duas camadas com *cross-validation* de nome entre páginas e trilha de auditoria, validado nos 4 PDFs descritos em §2. O `extract_dossier.py` orquestra via CLI no PoC. Em produção, a camada de orquestração fica a critério do escritório — opções viáveis incluem **n8n auto-hospedado** (escolha natural pelo perfil OSS + auditável, e a única dessas opções que avaliamos em dossiê), *cron* com pasta monitorada, ou *trigger* HTTP de uma aplicação web já existente; não construímos nenhuma delas no PoC. Referências: `02_tool_universe/05_n8n.md` e Diagrama 2 de `07_architecture/diagrams.md`.
